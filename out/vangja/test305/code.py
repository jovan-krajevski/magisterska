...
for point in pd.date_range(f"{year_start}-01-01", f"{year_end}-01-01"):
    ...
    parent_path = Path("./") / "out" / "vangja" / "test305"
    ...
    trend = LinearTrend(changepoint_range=1, allow_tune=True, tune_method="prior_from_idata")
    yearly = FourierSeasonality(365.25, 10, allow_tune=True, tune_method="prior_from_idata")
    weekly = FourierSeasonality(7, 3, allow_tune=True, tune_method="prior_from_idata")
    ...
    model_maps = []
    trend = LinearTrend(
        n_changepoints=0, allow_tune=True, tune_method="prior_from_idata", override_slope_mean_for_tune=slope_mean
    )
